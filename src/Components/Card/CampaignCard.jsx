    ...
    return (
        <Card className='border-white rounded-4'>
            <Card.Header className='bg-transparent py-3'>
                ...
                <p className={placeholder ? "placeholder w-50 py-3 rounded mb-0" : 'mb-0 text-secondary fs-14'}>
                    {
            ...
            <Card.Body className='row gy-3' onClick={clickFunction}>
                {
                    card_content.map((item, index) => (
                        <div className="col-12 d-flex flex-wrap mb-4" key={index}>
                            <div className={placeholder ? "placeholder w-75 py-2 rounded" : ''} style={{ width: '70%' }}>
                                {item.icon}
                                <span className='ps-2 text-secondary'>{item.title}</span>
                            </div>
                            <div className={placeholder ? "placeholder col py-2 rounded ms-2" : 'col'}>
                                <span>
                                    :
                                    <span className={placeholder ? "placeholder w-50 rounded" : ''}>
                                        {item.value}
                                    </span>
                                </span>
                            </div>
